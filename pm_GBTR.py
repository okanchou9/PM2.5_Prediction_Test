...
if __name__ == "__main__":
    ...
    # Remove the column "pm25_47" since no need to add this column to predict
    # Remove the column "temp_47" since no need to add this column to predict
    ...
    # gbtr = GBTRegressor(labelCol = target, featuresCol = "indexedFeatures", maxBins = 32, stepSize = 0.02, maxDepth = 8)
    gbtr = GBTRegressor(labelCol = target, featuresCol = "indexedFeatures")

    print("After GBTRegressor~~~~~")
    ...
    # #replace_to_1 = udf(lambda x, v: float(v) if x > 1 else x, DoubleType())

    # #temp1 = realPredictions.withColumn("pre_1", replace_to_1(col("prediction"), lit(1.0)))

    # #replace_to_0 = udf(lambda x, v: float(v) if x < 0 else x, DoubleType())

    # #temp2 = temp1.withColumn("pre_2", replace_to_0(col("pre_1"), lit(0.0)))

    # #temp2.select("ID", "pre_2").write.mode("overwrite").format("com.databricks.spark.csv").save("result")
